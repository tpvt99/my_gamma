...
def divide_trajectories(agent_id, content, trajectory_segment, prediction_frame_count):
...
for id in all_agent_id:
    divide_trajectories(agent_id=id, content=content, trajectory_segment=trajectory_segment,
                        prediction_frame_count=prediction_frame_count)

for index, id in enumerate(all_agent_id):
    if trajectory_segment[id] != []:
        traj_num, total_gamma_ade, total_gamma_fde = compute_accuracy(trajectory_segment[id], id)
